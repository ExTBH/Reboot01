...
enum ATMDBStatus _get_user(struct User *_Nonnull u, const char *buff,
                           const char *queryPath);
enum ATMDBStatus _fill_user(const PGresult *_Nonnull res,
                            struct User *_Nonnull u);
PGconn *conn = NULL;
...
void atmdb_close(void) { PQfinish(conn); }
...
enum ATMDBStatus _fill_user(const PGresult *_Nonnull res,
  ...
  unsigned long id = charToUll(id_str);
  if (id < 0) {
...
enum ATMDBStatus add_account(struct Account *_Nonnull acc) {
  if (acc == NULL) {
    return ATMDBStatusAccountsNull;
  }
  ...
  const char *doubleStr = doubleToChar(acc->balance);
  if (doubleStr == NULL) {
    return ATMDBStatusNotOK;
  }
  const char *params[5] = {buffer, acc->type, acc->country, acc->phone, doubleStr};
  PGresult *res = PQexecParams(conn, userQuery, 5, NULL, params, NULL, NULL, 0);
  if (PQresultStatus(res) != PGRES_TUPLES_OK) {
    const char *sqlState = PQresultErrorField(res, PG_DIAG_SQLSTATE);
...
struct Account **_fill_accounts(PGresult *res, int *_Nonnull count) {
  *count = PQntuples(res);
  ...
  for (int row = 0; row < *count; row++) {
    ...
    // strncpy(accounts[row]->date, PQgetvalue(res, row, 3), 10);
    ...
    strncpy(accounts[row]->country, PQgetvalue(res, row, 4), 20);
    strncpy(accounts[row]->phone, PQgetvalue(res, row, 5), 10);

  }
  return accounts;
}
enum ATMDBStatus get_accounts(struct User *_Nonnull u,
                              struct Account *_Nonnull *_Nonnull *_Nonnull accs,
                              int *_Nonnull count) {
  if (u == NULL) {
    return ATMDBStatusUsersNull;
  }
...
enum ATMDBStatus transfer_account(struct Account *_Nonnull acc,
                                  struct User *_Nonnull receiver) {
...
enum ATMDBStatus update_account(const unsigned long acc_id, const char country[20], const char phone[10]) {
    const char *userQuery = getQuery(QueryHelperUpdateAccount);
    // uint64 to string
    const char* buffer = ullToChar(acc_id);
    if (buffer == NULL) { return ATMDBStatusNotOK;}
    
    const char *params[3] = {phone, country, buffer};
    PGresult *res = PQexecParams(conn, userQuery, 3, NULL, params, NULL, NULL, 0);
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        fprintf(stderr, "Query execution failed: %s", PQresultErrorMessage(res));
        PQclear(res);
        free((char*)buffer);
...
enum ATMDBStatus update_account_balance(const unsigned long acc_id, const double balance) {
    const char *userQuery = getQuery(QueryHelperUpdateAccountBalance);
    // uint64 to string
    const char* buffer = ullToChar(acc_id);
    if (buffer == NULL) { return ATMDBStatusNotOK;}
    
    const char *doubleStr = doubleToChar(balance);
    if (doubleStr == NULL) { return ATMDBStatusNotOK;}
    
    const char *params[2] = {doubleStr, buffer};
    PGresult *res = PQexecParams(conn, userQuery, 2, NULL, params, NULL, NULL, 0);
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        fprintf(stderr, "Query execution failed: %s", PQresultErrorMessage(res));
        PQclear(res);
